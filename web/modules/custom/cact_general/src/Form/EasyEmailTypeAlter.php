...
class EasyEmailTypeAlter extends EasyEmailTypeForm {
  ...
  public function alterForm(&$form, FormStateInterface $form_state, $form_id) {
    ...
    $form['body_html']['message_product_pay'] = [
        ...
        '#default_value' => empty($message_product_pay) ? NULL : $message_product_pay['value'],
        '#format' => empty($message_product_pay) ? NULL : $message_product_pay['format'],
    ];
    $form['body_html']['message_product_free'] = [
        '#type' => 'text_format',
        '#rows' => 30,
        '#title' => $this->t('Message product (free) available token [cact:message_product_free]'),
        '#default_value' => empty($message_product_free) ? NULL : $message_product_free['value'],
        '#format' => empty($message_product_free) ? NULL : $message_product_free['format'],
    ];
