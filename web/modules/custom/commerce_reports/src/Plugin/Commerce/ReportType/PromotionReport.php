...
class PromotionReport extends ReportTypeBase {
  ...
  {
      return ['promotion_id' => BundleFieldDefinition::create('entity_reference')
        ->setLabel(t('Promotion'))
        ->setTargetEntityTypeId('commerce_promotion')
        ->setCardinality(1)
        ->setRequired(TRUE)
        ->setDisplayConfigurable('view', TRUE), 'promotion_label' => BundleFieldDefinition::create('string')
        ->setLabel(t('Name'))
        ->setDescription(t('The promotion name.'))
        ->setSettings([
          'default_value' => '',
          'max_length' => 255,
        ]), 'promotion_amount' => BundleFieldDefinition::create('commerce_price')
        ->setLabel(t('Promotion amount'))
        ->setCardinality(1)
        ->setRequired(TRUE)
        ->setDisplayConfigurable('view', TRUE), 'coupon_id' => BundleFieldDefinition::create('entity_reference')
        ->setLabel(t('Coupon'))
        ->setTargetEntityTypeId('commerce_promotion_coupon')
        ->setCardinality(1)
        ->setRequired(FALSE)
        ->setDisplayConfigurable('view', TRUE), 'coupon_code' => BundleFieldDefinition::create('string')
