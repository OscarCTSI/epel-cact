...
class OrderReport extends ReportTypeBase {
  ...
  {
      return ['order_type_id' => BundleFieldDefinition::create('entity_reference')
        ->setLabel(t('Order type'))
        ->setDescription(t('The order type.'))
        ->setSetting('target_type', 'commerce_order_type')
        ->setReadOnly(TRUE), 'amount' => BundleFieldDefinition::create('commerce_price')
        ->setLabel(t('Total Amount'))
        ->setDescription(t('The total amount of the order'))
        ->setCardinality(1)
        ->setRequired(TRUE)
        ->setDisplayConfigurable('view', TRUE), 'mail' => BundleFieldDefinition::create('email')
        ->setLabel(t('Contact email'))
        ->setDescription(t('The email address associated with the order.'))
        ->setCardinality(1)
        ->setSetting('max_length', 255)
        ->setRequired(TRUE)
        ->setDisplayConfigurable('view', TRUE), 'billing_address' => BundleFieldDefinition::create('address')
        ->setLabel(t('Address'))
