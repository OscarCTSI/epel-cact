...
class TaxReport extends ReportTypeBase {
  ...
  {
      return ['tax_amount' => BundleFieldDefinition::create('commerce_price')
        ->setLabel(t('Tax amount'))
        ->setCardinality(1)
        ->setRequired(TRUE)
        ->setDisplayConfigurable('view', TRUE), 'tax_type_id' => BundleFieldDefinition::create('entity_reference')
        ->setLabel(t('Tax type'))
        ->setSetting('target_type', 'commerce_tax_type')
        ->setCardinality(1)
        ->setRequired(TRUE)
        ->setDisplayConfigurable('view', TRUE), 'tax_type_label' => BundleFieldDefinition::create('string')
        ->setLabel(t('Name'))
        ...
        ->setRequired(TRUE)
        ->setDisplayConfigurable('view', TRUE), 'zone_id' => BundleFieldDefinition::create('string')
        ->setLabel(t('Zone ID'))
        ...
        ->setRequired(FALSE)
        ->setDisplayConfigurable('view', TRUE), 'zone_label' => BundleFieldDefinition::create('string')
        ...
        ->setRequired(FALSE)
        ->setDisplayConfigurable('view', TRUE), 'rate_id' => BundleFieldDefinition::create('string')
        ->setLabel(t('Rate ID'))
        ...
        ->setRequired(FALSE)
        ->setDisplayConfigurable('view', TRUE), 'rate_label' => BundleFieldDefinition::create('string')
  ...
  public function generateReports(OrderInterface $order) {
    ...
    foreach ($adjustments as $adjustment) {
      ...
      [$tax_entity_id, $zone_id, $rate_id] = explode('|', (string) $source_id) + [NULL, NULL, NULL];
      if ($tax_entity_id === null || $tax_entity_id === '' || $tax_entity_id === '0') {
        continue;
      }
