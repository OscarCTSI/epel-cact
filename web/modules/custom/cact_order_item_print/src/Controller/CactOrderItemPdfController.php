    ...
    if (!$code) {
      ...
      if(!empty($order_item)){
        ...
        if($order->getState()->value == 'completed' || in_array('administrator', $user_roles)){
              ...
              foreach ($order->getItems() as $oder_item_ref):
                ...
                if($product->id() == $product_ref->id()) {
                  ...
                  if (!isset($nominative_names)) {
                    $nominative_names = (!$oder_item_ref->get('field_name')->isEmpty())?$oder_item_ref->get('field_name')->getString():null;
                  } else {
                    $nominative_names .= (!$oder_item_ref->get('field_name')->isEmpty())?", ".$oder_item_ref->get('field_name')->getString():null;
                  }
            ...
            if (!isset($nominative_names)) {
              $nominative_names = (!$order_item->get('field_name')->isEmpty())?$order_item->get('field_name')->getString():null;
            }
